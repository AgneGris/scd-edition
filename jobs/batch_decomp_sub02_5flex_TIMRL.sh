...
LIVE_LOG=/rds/general/user/ag4916/ephemeral/thinfilm_logs_dir/sub02_5flex_TIMRL_live.txt
...
# Print job parameters
{
    ...
    echo "iterations     : 150"
    echo "Files:"
    echo "  /rds/general/project/thinfilmdata/live/forearm/raw_data/sub-02/day-01/sub-02_day-01_task-trapcombsameforce_ang-0_mvc-5flex_fing-TIMRL_time-1-1-25_reps-02_run-01.otb+"
    echo ""

    # Copy input files to local SSD for fast I/O
    echo "=== Copying input files to local SSD ==="
    t_copy=$SECONDS
    ORIG_FILES=("/rds/general/project/thinfilmdata/live/forearm/raw_data/sub-02/day-01/sub-02_day-01_task-trapcombsameforce_ang-0_mvc-5flex_fing-TIMRL_time-1-1-25_reps-02_run-01.otb+")
    LOCAL_FILES=()
...
} 2>&1 | tee -a "$LIVE_LOG" > $TMPDIR/sub02_5flex_TIMRL.txt

cp $TMPDIR/sub02_5flex_TIMRL.txt $PBS_O_WORKDIR/jobs/job_outputs/sub02_5flex_TIMRL.txt
