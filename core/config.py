"""
Configuration management for SCD Suite.
...
@dataclass
class PortConfig:
    """Configuration for a single port (electrode + processing settings)."""
    name: str
...
class ConfigManager:
    ...
    def _parse_port(self, data: Dict[str, Any]) -> PortConfig:
        ...
        # Using default filter/decomp configs for now when loading
        return PortConfig(
            name=data["name"],
            electrode=electrode,
