...
class DataHandler:
    ...
    def load_emg(self, path: Path, port_name: str, channels: List[int]) -> np.ndarray:
        ...
        if max_ch >= emg.shape[0]:
            raise ValueError(
                f"Channel {max_ch} exceeds available channels ({emg.shape[0]})"
            )
    ...
    def save_decomposition(self, path: Path, port_name: str):
        ...
        filters = np.array(
            [
                ...
                if mu.enabled and mu.filter is not None
            ]
        )
    ...
    def add_spike(self, port_name: str, mu_id: int, sample: int):
        ...
        if len(mu.timestamps) > 0 and np.any(
            np.abs(mu.timestamps - sample) < 0.005 * self.fsamp
        ):
    ...
    def delete_spike(
        self, port_name: str, mu_id: int, sample: int, tolerance_ms: float = 5.0
    ):
    ...
    def add_spikes_roi(
        ...
        peaks, _ = signal.find_peaks(
            segment, distance=min_distance, height=(y_min, y_max)
        )
